...
class AddFullTextSearch(Migration):
    ...
    def up(self):
        ...
        conn.statement(
            str(conn.raw("CREATE INDEX textsearchable_text_idx ON links USING GIN (textsearchable_text);  ")))
        conn.statement(
            str(conn.raw("CREATE INDEX textsearchable_title_idx ON links USING GIN (textsearchable_title);  ")))
