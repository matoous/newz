...
class AddFullTextSearchForFeeds(Migration):
    def up(self):
        ...
        conn.statement(
            str(
                conn.raw(
                    "ALTER TABLE feeds ADD COLUMN textsearchable_name tsvector; UPDATE feeds SET textsearchable_name = to_tsvector('english', name);"
                )
            )
        )
        conn.statement(
            str(
                conn.raw(
                    "ALTER TABLE feeds ADD COLUMN textsearchable_description tsvector; UPDATE feeds SET textsearchable_description = to_tsvector('english', description);"
                )
            )
        )
        conn.statement(
            str(
                conn.raw(
                    "CREATE OR REPLACE FUNCTION feed_create_tsvectors()   \n"
                    "    RETURNS TRIGGER AS $$\n"
                    "    BEGIN\n"
        ...
        conn.statement(
            str(
                conn.raw(
                    "CREATE INDEX textsearchable_name_idx ON feeds USING GIN (textsearchable_name);  "
                )
            )
        )
        conn.statement(
            str(
                conn.raw(
                    "CREATE INDEX textsearchable_description_idx ON feeds USING GIN (textsearchable_description);  "
                )
            )
        )
